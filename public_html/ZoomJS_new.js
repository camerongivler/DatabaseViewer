...
//            checkEdges();
    ...
    zoom.slider.hammer().on('drag', function(ev) {
        ...
        if (pos > -245 && pos < 208) {
            zoom.slider.css({'margin-left': pos, opacity: 1});
            zoom.leftBar.css({width: 251 + pos, opacity: 0.4});
            zoom.rightBar.css({width: 214 - pos, opacity: 0.4});
            zoom.scale = Math.pow(10, zoom.mult * (pos - 208) + log10(zoom.maxScale));
            transform(background.img, zoom.scale, zoom.scale, background.origX, background.origY);
        } else if (pos <= -245) {
            zoom.slider.css({'margin-left': -245, opacity: 1});
            zoom.leftBar.css({width: 6, opacity: 0.4});
            zoom.rightBar.css({width: 459, opacity: 0.4});
            transform(background.img, zoom.minScale, background.origX, background.origY);
        } else {
            zoom.slider.css({'margin-left': 208, opacity: 1});
            zoom.leftBar.css({width: 459, opacity: 0.4});
            zoom.rightBar.css({width: 6, opacity: 0.4});
            transform(background.img, zoom.maxScale, background.origX, background.origY);
        }
        background.width = background.origWidth * zoom.scale;
        background.height = background.origHeight * zoom.scale;
        //background.left = (win.width - background.width) * background.xPos + background.origX;
        //background.top = (win.height - background.height) * background.yPos + background.origY;
        background.left = background.img.offset().left;
        background.top = background.img.offset().top;
        makeThumb(false);
    });
    ...
    background.img.hammer().on('touch drag pinch', function(ev) {
        if (background.numTouches !== ev.gesture.touches.length) {
            ...
            //Log("1: " + ev.gesture.center.pageX + " " + ev.gesture.touches[0].pageX + " " + ev.gesture.touches[ev.gesture.touches.length - 1].pageX);
            background.numTouches = ev.gesture.touches.length;
            zoom.scale = zoom.currScale;
            background.origX = background.deltaX;
            background.origY = background.deltaY;
            ...
            $('#show3').css({top: background.yPos * 100 + "%", left: background.xPos * 100 + "%"});
            if (zoom.scale > zoom.maxScale) {
                zoom.scale = zoom.maxScale;
                zoom.currScale = zoom.scale;
                transform(background.img, zoom.maxScale, zoom.maxScale, background.origX, background.origY);
//                scaleTo(zoom.maxScale, true);
                ...
                zoom.currScale = zoom.scale;
                transform(background.img, zoom.minScale, zoom.minScale, background.origX, background.origY);
//                scaleTo(zoom.minScale, true);
            ...
            background.startX = ev.gesture.center.pageX;
            background.startY = ev.gesture.center.pageY;
            //background.width = background.origWidth * zoom.scale;
            //background.height = background.origHeight * zoom.scale;
            ...
            //background.xPos = (ev.gesture.center.pageX - background.left) / background.width;
            //background.yPos = (ev.gesture.center.pageY - background.top) / background.height;
            $('#show').css({top: background.top, left: background.left});
            $('#show2').css({top: background.top + background.height, left: background.left + background.width});
...
//            var scale = background.prevWidth * ev.gesture.scale / background.width;
...
//            background.img.css({width: background.width, height: background.height, top: background.top, left: background.left});
            // make this ev.gesture.startevent.pageX if possible
            background.deltaX = background.origX + ev.gesture.center.pageX - background.startX;
            background.deltaY = background.origY + ev.gesture.center.pageY - background.startY;
            zoom.currScale = zoom.scale * ev.gesture.scale;
            //background.width = background.origWidth * zoom.currScale;
            //background.height = background.origHeight * zoom.currScale;
            background.width = background.img.width();
            background.height = background.img.height();
            //background.left = (win.width - background.width) * background.xPos + background.deltaX;
            //background.top = (win.height - background.height) * background.yPos + background.deltaY;
            background.left = background.img.offset().left;
            background.top = background.img.offset().top;
            $('#show').css({top: background.top, left: background.left});
            $('#show2').css({top: background.top + background.height, left: background.left + background.width});
            transform(background.img, zoom.currScale, zoom.currScale, background.deltaX, background.deltaY);
            drawZoomBar(false, false);
            makeThumb(false);
        }
    });
    background.img.hammer().on('release', function(ev) {
...
var drawBackground = function() {
    ...
    if (image.aspect / win.width * win.height < 1) {
        zoom.maxScale = image.height / win.height;
        zoom.scale = win.width / win.height / image.aspect;
        background.origHeight = win.height;
        background.origWidth = win.height * image.aspect;
        background.img.css({height: background.origHeight, width: background.origWidth, top: 0, left: (win.width - win.height * image.aspect) / 2});
    } else {
        zoom.maxScale = image.width / win.width;
        zoom.scale = win.height / win.width * image.aspect;
        background.origWidth = win.width;
        background.origHeight = win.width / image.aspect;
        background.img.css({width: background.origWidth, height: background.origHeight, left: 0, top: (win.height - win.width / image.aspect) / 2});
    }
    transform(background.img, zoom.scale, zoom.scale, 0, 0);
    //background.height = background.origHeight * zoom.scale;
    //background.width = background.origWidth * zoom.scale;
    background.width = background.img.width();
    background.height = background.img.height();
...
var drawZoomBar = function(animate, opaque) {
    ...
    zoom.pos = (log10(zoom.currScale) - log10(zoom.maxScale)) / zoom.mult + 208;
    if (animate)
...
//    else {
//        background.img.css({top: background.top, left: background.left, width: background.width, height: background.height, '-webkit-transform': 'translateZ(0)'});
//    }
//    zoom.scale = pct;
//};
...
var MouseWheelHandler = function(e) {
    ...
    $('#show3').css({top: background.yPos * 100 + "%", left: background.xPos * 100 + "%"});
    Log(background.left + "    " + background.width);
    background.width += e.delta * zoom.scale / 5;
...
var transform = function(image, scaleX, scaleY, transformX, transformY) {
    image.css({transform: 'matrix3d(' + scaleX + ', 0, 0, 0, 0, ' + scaleY + ', 0, 0, 0, 0, 1, 0, ' +
                transformX + ', ' + transformY + ', 0, 1)',
        'transform-origin': (background.xPos * 100) + '% ' + (background.yPos * 100) + '%'});
};
